...
public interface UserMapper {
    ...
    // - Java 코드 내에서 SQL문을 직접 볼 수 있어 즉각적인 이해 가능
    // - @Select, @Insert, @Update, @Delete 어노테이션 사용
    @Select("SELECT * FROM users")
    List<User> findAll(); // UserService가 호출

    @Select("SELECT * FROM users where id = #{id}")
    User findById(Long id);

    @Insert("INSERT INTO users (username, email) value (#{username}, #{email})")
    ...
    // -> 새로 삽입된 행의 id 를 알 수 있게 되어 이후 작업에 해당 id 를 사용 가능
    // 만약 users 테이블의 id(pk) 를 수동으로 설정한다면 Options 어노테이션 필요x
